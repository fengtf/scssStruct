...
function parseTag(tag: string) {
  ...
  tag.replace(attrRE, (match) => {
    if (i % 2) {
      ...
      if (i === 0) {
        if (
          voidElementList.includes(match) ||
...
function splitSeletor(seletorStr: string, symbol: string, isBind: boolean) {
  const arr: Array<any> = [];
  let mt = isBind
    ? seletorStr.match(bindAttrValueRE)
    ...
    [AttrType.attrs, AttrType.bindAttrs].forEach((attrName) => {
      if (astObj[attrName][item.name]) {
        currentObj.selectorNames = currentObj.selectorNames.concat(
          splitSeletor(
            astObj[attrName][item.name],
            item.tag,
            attrName === AttrType.bindAttrs
          )
        );
      }
    });
...
function transTemplateAst(astObj: IDoc): any {
  ...
  if (
    (astObj.name.match(/^[A-Z]/) ||
      !htmlBlockTagList.concat(voidElementList).includes(astObj.name)) &&
    !astObj.attrs.class &&
    !astObj.attrs.id
  ) {
