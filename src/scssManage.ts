...
function completeSelectorName(scssObj: IScssAst, name: string = ''): string {
  if (scssObj.selectorNames.startsWith('&')) {
    return scssObj.parent
      ? completeSelectorName(
          scssObj.parent,
          scssObj.selectorNames + name.replace('&', '')
        )
...
const scssStr2Ast = (
  scssStr: string,
  root: IScssAst = JSON.parse(rootStrObj)
): IScssAst | null => {
...
const distinctChildren = (
  childrenArr: Array<IScssAst>,
  firstClassArr: string[]
) => {
  const obj: any = {};
  const distinctArr = [];
  ...
  for (let i = 0; i < len; i++) {
    if (!obj[childrenArr[i].selectorNames]) {
      ...
      if (
        !firstClassArr.includes(childrenArr[i].selectorNames) ||
        ((childrenArr[i].parent as IScssAst) &&
          (childrenArr[i].parent as IScssAst).selectorNames == 'wrapper')
      ) {
 ...
 */
const scssAstObj2Str = (obj: IScssAst, n = -1, firstClassArr: string[]) => {
  if (!obj) return '';
  let scssStr =
    (obj.comment || '') +
    (obj.isNew ? rnIndent(n) : obj.rnInfo.start || '') +
    obj.selectorNames +
    (obj.isNew ? ' ' : obj.rnInfo.startAfter) +
    '{';
  if (obj.rule) {
    scssStr += obj.rule || '';
  }
  if (obj.children.length) {
    const distinctArr = distinctChildren(obj.children, firstClassArr);

    distinctArr.forEach((childScssObj) => {
      scssStr += scssAstObj2Str(childScssObj, n + 1, firstClassArr);
    });
  }
...
const resetScss = (
  ...
  for (let i = 0; i < selectorNames.length; i++) {
    ...
    const matchIndex = scssObj.children.findIndex(
      (v) =>
        !v.hasMatch &&
        (selector === v.selectorNames ||
          (v.selectorNames.startsWith('&') &&
            selector === completeSelectorName(v)))
    );
    if (matchIndex === -1) {
        ...
        if (!scssObjChild.isKeyRule) {
          resetScss(
            ...
            templateObj.children
              .slice(0, index)
              .reduce((t, c) => t + c.selectorNames.length, 0)
          );
        }
      });
    }
...
export { scssStr2Ast, scssAstObj2Str, resetScss };
