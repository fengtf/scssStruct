...
export default (vueStr: string, scssStr?: string): string => {
  ...
  const templateObj = parseTemplate(
    '<wrapper>' + originTemplateStr + '</wrapper>'
  );
  // 解析 scss文本得到 scssAst对象
  const scssObj = scssStr2Ast(scssWrapper + originscssStr + '}');
  // 获取第一层级的className
  let firstClassArr: string[] = [];
  if (scssObj) {
    const firstClassOriginArr = scssObj.children[0].children;
    firstClassArr = firstClassOriginArr.map((item) => {
      return item.selectorNames;
    });
  }
  ...
  const newscssStr = scssAstObj2Str(
    (scssObj as IScssAst).children[0],
    -1,
    firstClassArr
  )
    .trim()
    .slice(scssWrapper.length, -1)
    .trimLeft();
  // 将 scss文本写入本地文件
  let newVueFile = '';
  if (typeof scssStr === 'string') {
    ...
    if (originscssMt) {
      const scssMt = vueStr.match(
        /([\s\S]*)(<style[^>]*>)([\s\S]*)(<\/style>\s*)/
      );
      if (scssMt) {
        newVueFile =
          scssMt[1] +
